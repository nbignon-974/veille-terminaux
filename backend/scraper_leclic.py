...
async def run_scrape(on_progress=None) -> list[PhoneData]:
    ...
    async with async_playwright() as pw:
        ...
        # link read from the first page (which, if pagination renders late, comes back
        # as 1 and silently collects only page 1). An out-of-range page returns either
        # a clamped duplicate page or nothing — both give 0 new and stop the loop.
        all_raw: list[dict] = []
